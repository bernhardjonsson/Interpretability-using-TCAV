...
class ModelWrapper(six.with_metaclass(ABCMeta, object)):
  ...
  def _make_gradient_tensors(self):
    ...
    # Adding the logit layer
    self.bottlenecks_gradients["logit"] = tf.gradients(
          ys=self.loss, xs=self.ends["logit"])[0]
  ...
  def get_gradient(self, acts, y, bottleneck_name, example):
    ...
    Returns:
      the gradient array.
    """
    if bottleneck_name == "logit":
        return self.sess.run(self.bottlenecks_gradients["logit"], {
            self.ends["logit"]: acts,
            self.y_input: y
        })
    else:
  ...
  def run_examples(self, examples, bottleneck_name):
    ...
    if bottleneck_name != "logit":
        ...
        return self.sess.run(self.ends["logit"],
                         {self.ends['input']: examples})
