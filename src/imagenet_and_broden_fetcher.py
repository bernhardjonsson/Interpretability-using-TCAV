...
kBrodenColorsPath = "broden1_224/images/"
kMinFileSize = 10000
...
"""Moves all colors in a downloaded Broden to our working folder.
...
Assumes that you manually downloaded the broden dataset to broden_path.
  ...
  saving_path: String.Where we'll save the images. Saves under
    path/color_name.
  color_name: String representing color name i.e red-c
  number_of_images: Integer.Number of images to move
"""
def download_color_to_working_folder(broden_path, saving_path, color_name,
  ...
  # Get images from broden
  broden_colors_path = os.path.join(broden_path, kBrodenColorsPath)
  ...
  for root, dirs, files in os.walk(broden_colors_path):
    # To find the information about the image color we find the _color suffixed images
    color_files += [
        os.path.join(root,a.split("-c_")[1].replace('_color.png','.jpg')) for a in files if (a.startswith(color_name) and "color" in a)
    ]
  ...
  while save_number > 0:
    for file in color_files:
      path_file = file
      color_saving_path_file = os.path.join(color_saving_path, os.path.split(file)[1])
      tf.io.gfile.copy(
